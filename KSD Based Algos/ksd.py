...
def get_K_row(samples, gradients, kernel_type, h, index=-1):
    ...
    if kernel_type == 'rbf':
        ...
        kernel_grads = (2 / h) * torch.mul(kernel_values.unsqueeze(1), pdiffs)
...
def get_sequential_KSDs(samples,
    ...
    return [x for x in cum_sums.sqrt().divide(torch.arange(1,cum_sums.shape[0]+1).to(cum_sums.devicem
                                                                                     ))]
