...
def em_algorithm(samples,parameters,N_components = 2, N_iter = 10):
...
def ksd_index(armSamples,parameters,lambda_,time,N_iter = 10,N_components = 2):
    """
...
def ksd_ucb(samples,parameters,lambda_,time,N_iter,N_arms =2):
    """
    :[i]: arm i
    ...
    for i in range(N_arms):
        ...
        parameters_arm = parameters[i]
        ### Compute KSD
        ksd_indices[i],parameters_returned = ksd_index(armSamples,parameters_arm,lambda_,time,N_iter = N_iter) 
...
if RUN_EXP:
    for mc in tqdm.tqdm(range(N_MC)):
        ...
        for _ in range(N_arms):### pull arm 10 times for each method
            for j in range(10):
        ...
        for t in tqdm.tqdm(range(T)):
            ...
            arm,parameters_thompson = thompson_sampling(total_data[0],parameters_thompson,N_components = N_components, N_iter = N_iter,N_arms=N_arms)
            ...
            # arm,parameters_ksd = ksd_ucb(total_data[1],parameters_ksd,lambda_=1,time = t,N_iter = N_iter,N_arms=N_arms)
            # reward = pull_arm(arm,mean_mixtures,variance_mixtures,weights)
